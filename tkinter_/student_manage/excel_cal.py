...
def cal_total_score(data):
    ...
    for index, row in data.iterrows():
        ...
        id2score[row.values[0]] = 100-deduct_points
    return id2score
...
# 将总分和排名写回到表格中
def write_total_score(id2score, id2sort, data, filename, sheet_name):
    # 加载工作簿
    ...
    for index, row in data.iterrows():
        ...
        ws.cell(row=index + 2, column=17, value=id2score[row.values[0]])  # +2是因为 DataFrame 索引从 0 开始，Excel 从 1 开始，且第一行通常是表头
